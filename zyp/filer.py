        ...
        if request.FILES:
            ...
            if myf:
                ...
                for chunk in myf.chunks():
                    dest.write(chunk)
                dest.close()
                return ret_success({"f_name": myf.name, "f_suffix": suffixinfo, "uid": uid, "size": myf.size,
...
def switch_file_type(request: HttpRequest) -> JsonResponse:
    ...
    try:
        ...
        return ret_error(e)
def argues_loss_delete(dta, argues) -> pd.DataFrame:
    data = []
    for x in dta.index:
        flag = 0
        for y in argues:
            if (pd.isnull(dta.loc[x, y]) or dta.loc[x, y] == ' ' or dta.loc[x, y] == ''):
                flag = flag + 1
                break
        if flag == 0:
